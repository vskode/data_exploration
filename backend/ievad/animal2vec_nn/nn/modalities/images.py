...
class ImageEncoder(ModalitySpecificEncoder):
    ...
    def __init__(
        ...
        torch.tnn.init.xavier_uniform_(w.view([w.shape[0], -1]))

        if modality_cfg.embed_dim != embed_dim:
            local_encoder = tnn.Sequential(
                local_encoder,
                tnn.Linear(modality_cfg.embed_dim, embed_dim),
            )

        project_features = tnn.Identity()

        pos_embed = tnn.Parameter(
