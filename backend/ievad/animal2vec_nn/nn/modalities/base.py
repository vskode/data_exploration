...
from ievad.animal2vec_nn.nn import Modality
...
class ModalitySpecificEncoder(nn.Module):
    ...
    def local_features(self, features):
        # logger.info("\n\n features.size(): {}".format(features.size()))  # torch.Size([12, 8000])
        ...
        if x.dim() == 2:
            x = x.unsqueeze(1)
        # logger.info("\n\n x.size(): {}".format(x.size()))  # torch.Size([12, 512, 200])
        x = self.project_features(x)
        # logger.info("\n\n x.size(): {}".format(x.size()))  # torch.Size([12, 200, 768])
        return x
    ...
    def contextualized_features(
        ...
        if mask:
            if clone_batch > 1:
                # logger.info("\n\n pre repeat interleave x.size(): {}".format(x.size()))  # torch.Size([12, 200, 768])
                x = x.repeat_interleave(clone_batch, 0)
                # logger.info("\n\n post repeat interleave x.size(): {}".format(x.size()))  # torch.Size([96, 200, 768])
            ...
            # logger.info("\n\n padding_mask.size(): {}".format(padding_mask.size()))
            # logger.info("\n\n padding_mask: {}".format(padding_mask))
