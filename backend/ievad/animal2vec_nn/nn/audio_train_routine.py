...
def animal2vec_audio_main(cfg: FairseqConfig) -> None:
    ...
    # if torch.__version__ >= version.parse("2.0"):
    #     logger.info("We try to compile the model using pytorch 2.0 .compile")
    ...
    if cfg.dataset.combine_valid_subsets:
        task.load_dataset(cfg.dataset.valid_subset, combine=True, epoch=1)
    else:
    ...
    logger.info(
        "max tokens per device = {} and max batch size per device = {}".format(
    ...
    # logger.info("Starting the export of the embeddings")
    # _ = validate(cfg, trainer, task, epoch_itr, ["train"], final_eval=True)
...
def train(
    ...
    progress.print(stats, tag=cfg.dataset.train_subset, step=num_updates)
    # if cfg.task.verbose_tensorboard_logging:
    #     tensorboard_writer_method = getattr(progress, "_writer", None)
    #     nn.log_metrics(trainer, tensorboard_writer_method,
    #                    cfg, accumulated_samples if len(accumulated_samples) > 0 else samples,
    #                    num_updates, train_str=cfg.dataset.train_subset)
    #     del accumulated_samples
...
def validate_and_save(
    ...
    if do_validate:
        ...
        trainer._criterion.can_sum = True
        trainer.criterion.can_sum = True
    ...
    if args.profile:
        with torch.cuda.profiler.profile():
            with torch.autograd.profiler.emit_nvtx():
                distributed_utils.call_main(cfg, animal2vec_audio_main)
    else:
        distributed_utils.call_main(cfg, animal2vec_audio_main)
